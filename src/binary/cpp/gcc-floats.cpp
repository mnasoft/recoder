...
#include <random>
#include <limits>
#include <quadmath.h> // Для __float128
...
int main() {
    ...
    // Генерация случайных значений для quad double (__float128)
    for (int i = 0; i < 5; ++i) {
        __float128 value = static_cast<__float128>(std::numeric_limits<long double>::min()) +
                           (static_cast<__float128>(std::numeric_limits<long double>::max()) - 
                            static_cast<__float128>(std::numeric_limits<long double>::min())) * 
                           static_cast<__float128>(rd()) / rd.max();
        outFile.write(reinterpret_cast<char*>(&value), sizeof(value));
        //txtFile << value << " ";
        char quadStr[128];
        snprintf(quadStr, sizeof(quadStr), "%.30Qg", value);
        
        //quadmath_snprintf(quadStr, sizeof(quadStr), "%.30Qg", value);
        txtFile << quadStr << " ";
    }
    txtFile << std::endl;
