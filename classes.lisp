...
(defclass trend ()
  ...
  (:documentation "Trend -- класс служащий для предоставления интерфейса к файлу-тренду, содержащему записи аналоговых и дискретных параметров;
Файл-тренд имеет следующую структуру:
1 Заголовок тренда;
2 Список дескрипторов (описаний) аналоговых сигналов, 
  представляющий из себя последовательно записанные записи дескрипторов аналоговых сигналов;
3 Список дескрипторов (описаний) дискретных сигнало, 
  представляющий из себя последовательно записанные записи дескрипторов дискретных сигналов;
4 Список записей, состоящий из последовательно записанных списка аналоговых сигналов и упакованного списка дискретных сигналов;
;
...
;
Аналоговый сигнал хранится в закодированном целочисленном виде значения длиной 2 байта
Значение закодированного целым числом i аналогового сигнала вычисляется по формуле:
rez=analog-LowLimit+(i*(analog-HighLimit-analog-LowLimit)/65535)
;
Дискретный сигнал - значенние, пердставленное одним битом информации 0|1;
При записи в тренд на восемь дискретных сигналов отводится один байт;
Сигналы упаковываются слева-направо (справа-направо ?)"))
