...
(defun read-trd-file-float(in)
  "Выполняет чтение long-long из потока in"
  (let ((rez 0))
    (dotimes (i 4 (ie3fp:decode-ieee-float rez))
      (setf (ldb (byte 8  (* 8 i)) rez) (read-byte in)))))

(defun read-trd-file-double(in)
  "Выполняет чтение long-long из потока in"
  (let ((rez 0))
    (dotimes (i 8 (ie3fp:decode-ieee-double rez))
      (setf (ldb (byte 8  (* 8 i)) rez) (read-byte in)))))

(defun read-trd-file-quad (in)
  "Выполняет чтение long-long из потока in"
  (let ((rez 0))
    (dotimes (i 16 (ie3fp:decode-ieee-quad rez))
      (setf (ldb (byte 8  (* 8 i)) rez) (read-byte in)))))
